...
First we need to activate logging on the entities that we want logged (for obius security reasons we donít log entity changes by default) to do that by adding the `@Gedmo\Loggable` annotation to our php class, which should then look something like:
